...
// 模块加载后，对nodeJs的模块的核心部分进行引导，即内核引导
...
(function bootstrapNodeJSCore(process,
  ...
  const isMainThread = internalBinding('worker').threadId === 0; // 判断是否是主进程
  
  /**
   * 启动引导函数
   */
  function startup() {
    const EventEmitter = NativeModule.require('events'); // 获取event对象中的触发器对象

    const origProcProto = Object.getPrototypeOf(process); // 获取process对象的原型
    Object.setPrototypeOf(origProcProto, EventEmitter.prototype); // 将process对象的原型设置为EventEmitter的原型

    EventEmitter.call(process); // 使process具有EventEmitter的特性
